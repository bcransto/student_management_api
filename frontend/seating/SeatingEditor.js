...
const SeatingEditor = ({ classId, periodId, onBack, onView, navigateTo, startInDraft, onDraftConsumed }) => {
  ...
  const getEditorTitle = () => {
    ...
    // No current period yet - single line
    return React.createElement(
      "div",
      { 
        style: { 
  ...
  const handleMakeActive = async () => {
    ...
    // Get the actual current active period (to show in the message)
    ...
    // Get the viewed period name (the one we're currently looking at)
    const viewedPeriodName = classInfo?.current_seating_period?.name || "this period";
    ...
    try {
      ...
      const periodId = classInfo?.current_seating_period?.id;
      if (!periodId) {
        alert("No period ID found. Please refresh and try again.");
        return;
      }
  ...
  // Toggle the one-off (untracked) status of the chart being viewed.
  ...
  const handleToggleOneOff = async () => {
    ...
    const isOneOff = period.is_tracked === false;

    setIsCreatingPeriod(true);

    try {
      if (isOneOff && period.end_date === null) {
        // Restoring an OPEN one-off to tracked makes it the current chart;
        // make_current also ends any other tracked current period
        await window.ApiModule.request(`/seating-periods/${period.id}/make_current/`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        });
      } else {
        await window.ApiModule.request(`/seating-periods/${period.id}/`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ is_tracked: isOneOff }),
        });
      }

      // Update local state so the pill and bolt re-render immediately
      setClassInfo((prev) => ({
        ...prev,
        current_seating_period: { ...prev.current_seating_period, is_tracked: isOneOff },
      }));
    } catch (error) {
      console.error("Error toggling one-off status:", error);
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        onView && React.createElement(
          ...
          React.createElement("i", { className: "fas fa-eye", style: { fontSize: "14px", color: "white" } })
        ),
