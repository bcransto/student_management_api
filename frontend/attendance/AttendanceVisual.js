...
const AttendanceVisual = ({ classId, date, onBack, navigateTo }) => {
  ...
  // State for chart selector (tracked current chart + open one-offs)
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (showChartDropdown && !event.target.closest(".av-chart-dropdown-btn")) {
    ...
    try {
      ...
      if (cached) {
        ...
        if (classData.roster && Array.isArray(classData.roster)) {
          studentList = classData.roster.map((roster) => ({
            id: roster.student,
            rosterId: roster.id,
        ...
        try {
          ...
          // Build the selectable chart list: the tracked current chart plus any
          // OPEN one-offs (a one-off is where "kids sit today" - e.g. a sub day).
          selectableCharts = buildSelectableCharts(periods);
          setAvailableCharts(selectableCharts);

          // Default to the tracked current chart; fall back to the most recent
          // open one-off, then any most-recent period. Honor an explicit pick.
          const preferred =
            ...
            if (fullPeriod.layout_details) {
              ...
              if (layoutData.tables) {
                layoutData.tables.forEach((table) => {
                  console.log(
                    `Table ${table.table_number} (ID: ${table.id}): ${table.seats?.length || 0} seats`
                  );
                });
              }
        ...
        try {
          ...
          classData.roster.forEach((roster) => {
            ...
            if (consecutiveCount > 0) {
              ...
              console.log(
                `Student ${studentId} (roster ${rosterId}): ${consecutiveCount} historical consecutive absences`
              );
            }
          });
  ...
  // Handle chart selection - swaps only the seating surface (layout +
  // assignments). Attendance is stored per roster+date, so it is untouched.
  ...
  const handleSave = async () => {
    ...
    if (
      initialLoadComplete &&
      Object.values(attendanceRecords).every((r) => r.status === "present")
    ) {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        // Chart selector - only shown when an open one-off exists alongside
        // the current chart (otherwise there is nothing to choose)
        availableCharts.length > 1 &&
          React.createElement(
            ...
            React.createElement(
              "button",
              {
                className: "av-chart-dropdown-btn",
        ...
        React.createElement(
          "button",
          {
            onClick: () => {
              if (
                hasUnsavedChanges &&
                !confirm("You have unsaved changes. Switch to Special Points mode?")
              )
                return;
              window.location.hash = "special-points/visual/" + classId;
            },
            style: {
              background: "#6366f1",
              color: "white",
            ...
            title: "Switch to Special Points mode",
          },
      ...
      layout
              ...
              React.createElement(
                ...
                table.seats?.map((seat) => {
                  ...
                  const rosterId = student
                    ? students.find((s) => s.id === studentId)?.rosterId
                    : null;

                  // Get attendance status for this student
                  const attendanceStatus =
                    rosterId && attendanceRecords[rosterId]
                    ...
                    React.createElement(
                      ...
                      rosterId && consecutiveAbsences[rosterId] && consecutiveAbsences[rosterId] > 0
                        ? React.createElement(
                            ...
                            consecutiveAbsences[rosterId] >= 11
                              ? ">10"
                              : String(consecutiveAbsences[rosterId])
                          )
                        : null,
        ...
        : React.createElement(
            ...
            React.createElement(
              "p",
              null,
              "This class needs a seating layout for visual attendance."
            )
          )
    ),
