...
from students.models import (
    User, Class, Student, ClassRoster, 
    ClassroomLayout, ClassroomTable, TableSeat, 
...
class Command(BaseCommand):
    ...
    def create_default_layouts(self, dry_run):
        ...
        for teacher in teachers:
            ...
            if not dry_run:
                ...
                table_positions = [
                    (1, 2, 2), (2, 5, 2), (3, 8, 2),  # Back row
                    (4, 2, 5), (5, 5, 5), (6, 8, 5),  # Front row
                ]
                ...
                for table_num, x_pos, y_pos in table_positions:
                    ...
                    seat_positions = [
                        (1, 0.25, 0.25), (2, 0.75, 0.25),  # Top seats
                        (3, 0.25, 0.75), (4, 0.75, 0.75),  # Bottom seats
                    ]
    ...
    def migrate_existing_seating_data(self, dry_run):
        ...
        for class_obj in classes_with_seating:
            ...
            if not dry_run:
                ...
                default_layout = ClassroomLayout.objects.filter(
    ...
    def create_current_seating_periods(self, dry_run):
        ...
        classes_without_periods = Class.objects.filter(
            seating_periods__isnull=True
        ).distinct()
    ...
    def print_migration_summary(self):
        ...
        classes_with_seating = Class.objects.filter(
            roster__seat_number__isnull=False
        ).distinct().count()
        self.stdout.write(f'Classes with existing seating data: {classes_with_seating}')
        
        # Count total roster entries with seats
        roster_with_seats = ClassRoster.objects.filter(
