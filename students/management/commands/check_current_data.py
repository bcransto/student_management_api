...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if has_old_fields:
            ...
            try:
                from .models import SeatingPeriod, SeatingAssignment
        ...
        students_multi_class = Student.objects.filter(
            enrollments__is_active=True
        ).annotate(
            class_count=Count('enrollments')
        ).filter(class_count__gt=1).count()
