...
class ClassroomTable(models.Model):
    ...
    table_shape = models.CharField(max_length=20, choices=[
    ...
    rotation = models.PositiveIntegerField(default=0, choices=[
...
class LayoutObstacle(models.Model):
    ...
    obstacle_type = models.CharField(max_length=30, choices=[
...
class Class(models.Model):
    ...
    def get_seating_history_for_student(self, student):
        ...
        return SeatingAssignment.objects.filter(
            seating_period__class_assigned=self,
            roster_entry__student=student
    ...
    def get_students_who_sat_together(self, student1, student2):
        ...
        periods_with_both = SeatingPeriod.objects.filter(
            class_assigned=self,
            seating_assignments__roster_entry__student__in=[student1, student2]
        ).annotate(
            student_count=models.Count('seating_assignments__roster_entry__student', distinct=True)
        ...
        for period in periods_with_both:
            ...
            if s1_assignment and s2_assignment:
                # Check if same group
                same_group = (s1_assignment.group_number and s2_assignment.group_number and
                            s1_assignment.group_number == s2_assignment.group_number)
...
class SeatingAssignment(models.Model):
    ...
    seat_id = models.CharField(max_length=20, help_text="Seat ID from classroom layout (e.g., '1-2' for table 1, seat 2)")
    ...
    def __str__(self):
        ...
        return f"{self.roster_entry.student.get_full_name()} - {self.seating_period.name}, Seat {self.seat_id}{group_info}"
    ...
    def group_members(self):
        ...
        return SeatingAssignment.objects.filter(
            seating_period=self.seating_period,
    ...
    def table_mates(self):
        ...
        table_assignments = SeatingAssignment.objects.filter(
            seating_period=self.seating_period,
            seat_id__startswith=f"{table_num}-"
        ).exclude(id=self.id)
